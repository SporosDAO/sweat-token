...
export default function ProposalCard(props: any) {
  ...
  const { proposer, proposalType, description, sponsor, sponsored, cancelled, status, votes, votingStarts, dao } =
    proposal
  // console.debug({ dao })
  ...
  const votesFor = votes?.reduce(
    (result: any, item: { vote: any; weight: any }) => result + (item.vote ? ethers.utils.formatEther(item.weight) : 0),
    0
  )
  const votesTotal = votes?.reduce(
    (result: any, item: { vote: any; weight: any }) => result + ethers.utils.formatEther(item.weight),
    0
  )

  const votesForPercentage = (votesFor * 100) / votesTotal
